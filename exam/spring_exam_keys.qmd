...
1.  Формулировка метода тяжелого шарика (Поляка) для гладкой выпуклой/сильно выпуклой функций. Характер сходимости в терминах $\mathcal{O}$ от числа итераций метода.
    ...
    :::{.callout-tip appearance="simple"}
    Задача: $f(x) \rightarrow \min\limits_{x \in \mathbb{R}^d}, \ f(x), \lambda(A) \in [\mu; L]$.
    $$
    x_{k+1} = x_{k} −\alpha\nabla f(x_{k}) + \beta(x_k−x_{k−1}), \qquad 0 < \beta < 1.
    $$
...
1.  Критерий проекции точки на выпуклое множество (Неравенство Бурбаки-Чейни-Гольдштейна).
    ...
    :::{.callout-tip appearance="simple"}
    Проекция $\text{proj}_S(x)$ точки $x$ на замкнутое и выпуклое множество $S$ удовлетворяет:
    $$
...
1.  Характер сходимости проксимального градиентного метода для гладких сильно выпуклых функций $f$ в терминах $\mathcal{O}$ от числа итераций метода.
    ...
    :::{.callout-tip appearance="simple"}
    Рассматривается задача: $\varphi(x) \rightarrow \min\limits_{x \in \mathbb{R}^n}$, где $\varphi(x)  = f(x) +r(x)$, $f(x)$ - гладкая сильно выпуклая, $r(x)$ - негладкая выпуклая, проксимально дружественная.
    $$
    \|x_k - x^*\|^2 \sim \mathcal{O} \left( \left(1 - \frac{\mu}{L}\right)^k \right)
    $$
...
1.  Метод SAG.
    ...
    $\mathbb{E}\left[f(x_{\text{mean}}^{(k)}) - f^*\right] \leq \frac{48n|f(x^{(0)}) - f^*| + 128L\|x^{(0)} - x^*\|^2}{k} = O(\frac{1}{k})$
...
1.  Метод Muon.
    ...
    $$
    где $G_t = U \Sigma V^\top$ - сингулярное разложение матрицы градиентов соответствующей текущему слою.
    :::
...
1.  Идея dual decomposition.
    ...
    $$
    \begin{aligned}
    x^{k}_i &\in \arg\min\limits_{x_i}\left(f_i(x_i) + (u^{k-1})^TA_ix_i\right), \quad i = \overline{1,B} \\
...
1.  Метод ADMM.
    ...
    \end{aligned}
    $$
    После добавления штрафа за выход из бюджетного множества имеем $f(x) + g(z) + \frac{\rho}{2}\|Ax + Bz - c\|^2 \rightarrow \min\limits_{Ax + Bz = c}$, где $\rho > 0$ - параметр. Тогда функция Лагранжа имеет вид:
    $$
    ...
    **Доказательство:**
    ...
    1. Определим следующую $L$-гладкую выпуклую функцию: $f(x) = \frac{L}{4}\left(\frac{1}{2} x^T A x - e_1^T x \right) = \frac{L}{8} x^T A x - \frac{L}{4} e_1^T x.$
        ...
        \end{bmatrix} \begin{bmatrix}
            ...
            \vdots \\
            x_{n}^* \\
        \end{bmatrix} = \begin{bmatrix} 1 \\ 0 \\ 0 \\ \vdots \\ 0 \end{bmatrix} \quad \begin{cases} 2x_1^* - x_2^* = 1 \\ -x_i^* + 2x_{i+1}^* - x_{i+2}^* = 0, \; i = 1, \ldots, n-2 \\ -x_{n-1}^* + 2x_n^* = 0 \end{cases}
        $$
...
1.  Вывод формул обновления оценок обратного гессиана и гессиана квазиньютоновских методов SR-1, DFP, BFGS.
    ...
    2. **Davidon-Fletcher-Powell Update (DFP)**
        ...
        **Sherman-Morrison Formula:**
        ...
        $$ (1 + a \Delta y_k ^T C \Delta y_k) C_k \Delta y_k + (bd_k^T\Delta y_k - 1)d_k = 0 \Leftrightarrow a = -\frac{1}{\Delta y_k ^T C \Delta y_k}, b = \frac{1}{\Delta y_k^T d_k}$$
    ...
    3. **Broyden-Fletcher-Goldfarb-Shanno update**
        ...
        $$ (1 - a \Delta y_k^T d_k)\Delta y_k - (1 + bd_k^TB_k d_k)B_kd_k = 0 \Leftrightarrow a = \frac{1}{y_k^T d_k}, b = -\frac{1}{d_k^TB_k d_k} $$
...
<!-- 1.  Теорема о сходимости метода проекции градиента для сильно выпуклых гладких функций.
    ...
    **Доказательство**
    ...
    3. Используем гладкость и сильную выпуклость: 
        $$
        \begin{aligned}
...
1.  Теорема о сходимости проксимального градиентного метода для сильно выпуклой гладкой функции.
    ...
    **Доказательство**
    ...
    1. Рассмотрим расстояние до решения и используем свойство стационарности:
        $$
        \begin{aligned}
    ...
    1. Используем гладкость и сильную выпуклость: 
        $$
        \begin{aligned}
        \text{smoothness} \;\; &\|\nabla f(x_k)-\nabla f (x^*)\|_2^2 \leq 2L\left(f(x_k)-f(x^*)-\langle\nabla f (x^*),x_k -x^*\rangle \right) \\
        \text{strong convexity} \;\; & - \langle \nabla f(x_k) -  \nabla f(x^*), x_k - x^* \rangle \leq \\
    ...
    1. Из выпуклости $f$: $f(x_k) - f(x^*) - \langle \nabla f(x^*), x_k - x^* \rangle \geq 0$. Следовательно, если мы используем $\alpha \leq \frac1L$:
        $$
        \begin{aligned}
        \|x_{k+1} - x^*\|^2_2 &\leq (1 - \alpha \mu)\|x_k - x^*\|^2 \\
        \|x_k - x^*\|_2^2 &\leq (1 - \alpha \mu)^k \|x_0 - x^*\|_2^2
